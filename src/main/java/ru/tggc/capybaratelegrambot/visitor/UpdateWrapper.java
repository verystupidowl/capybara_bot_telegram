...
public record UpdateWrapper(Update update) {
    ...
    public CompletableFuture<Response> accept(UpdateDispatcherVisitor visitor) {
        return Optional.ofNullable(update.message())
                .map(m -> receiveMessage(visitor, m))
                .orElseGet(() -> receiveCallback(visitor));
    }

    private CompletableFuture<Response> receiveCallback(UpdateDispatcherVisitor visitor) {
        ...
        if (callbackQuery != null) {
            ...
            return visitor.onCallback(callbackQuery)
                    .thenApply(r -> r.andThen(Response.of(answerCallbackQuery)));
        }
        return CompletableFuture.completedFuture(Response.empty());
    }

    private CompletableFuture<Response> receiveMessage(UpdateDispatcherVisitor visitor, Message m) {
