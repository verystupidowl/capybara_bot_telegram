...
import ru.tggc.capybaratelegrambot.DataBase.CapybaraDAO;
import ru.tggc.capybaratelegrambot.DataBase.Request;
...
import ru.tggc.capybaratelegrambot.capybara.CapybaraPhoto;
import ru.tggc.capybaratelegrambot.capybara.Username;
import ru.tggc.capybaratelegrambot.capybara.job.Jobs;
import ru.tggc.capybaratelegrambot.capybara.job.MainJob;
import ru.tggc.capybaratelegrambot.capybara.properties.CapybaraRace;
import ru.tggc.capybaratelegrambot.capybara.properties.WeddingGiftDate;
import ru.tggc.capybaratelegrambot.domain.dto.PhotoDto;
import ru.tggc.capybaratelegrambot.exceptions.CapybaraException;
import ru.tggc.capybaratelegrambot.exceptions.CapybaraHasNoMoneyException;
import ru.tggc.capybaratelegrambot.exceptions.CapybaraNullException;
import ru.tggc.capybaratelegrambot.handler.AbstractHandler;
import ru.tggc.capybaratelegrambot.keyboard.InlineKeyboardCreator;
import ru.tggc.capybaratelegrambot.keyboard.SimpleKeyboardCreator;
...
//                String reduce = userCapybara.stream().map(Capybara::getName).reduce("", (c1, c2) -> c1 + "\n" + c2);
    ...
    public synchronized void serveTextCommandsPublic(Message message, Bot bot) {
        ...
        switch (message.text().toLowerCase(Locale.ROOT)) {
            ...
            case "взять капибару" -> takeCapybara(message);
            case "моя капибара", "/my_capybara", "/my_capybara@capybara_pet_bot" -> {
                ...
                try {
                    ...
                    bot.execute(new SendPhoto(chatId, capybara.getCapybaraPhoto().toUrl())
                            .caption(text.getMyCapybara(capybara, message))
    ...
    @CheckCapybara(checkType = CheckType.CHECK_EXISTS)
    private void takeCapybara(Message message) {
        String userId = message.from().id().toString();
        PhotoDto photoDto = capybaraService.saveCapybara(userId, chatId);
